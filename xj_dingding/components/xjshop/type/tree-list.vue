	...
	export default {
		...
		methods: {
			...
			getGoodsListForType(typeItem) {
				...
				//获取当前代下单客户的报价单id
				let curClientObj = uni.getStorageSync('daiClientObj');
				if(curClientObj && curClientObj.quotationId){
					url += '&quotationId=' + curClientObj.quotationId;
				}
