...
import com.javaboot.quartz.config.DbParamConfig;
import com.javaboot.quartz.service.ISysJobService;
import com.javaboot.quartz.util.BatCommandUtil;
 ...
 */
@Component("jbTask")
public class JavaBootTask {
    private static final Logger log = LoggerFactory.getLogger(JavaBootTask.class);
    @Autowired
    ...
    public void recoverDBFile(String fileId) {
        log.info("------------------执行定时任务------------jbTask.recoverDBFile，参数:" + fileId);
    ...
    public void backupDB() {
        log.info("------------------执行定时任务------------jbTask.backupDB");
        try {
            initMysqlDir();
            //备份数据库
