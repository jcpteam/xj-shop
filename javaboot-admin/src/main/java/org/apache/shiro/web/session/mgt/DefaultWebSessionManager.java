...
public class DefaultWebSessionManager extends DefaultSessionManager implements WebSessionManager {
    ...
    private void storeSessionId(Serializable currentId, HttpServletRequest request, HttpServletResponse response) {
        if(!"/captcha/captchaImage".equals(request.getRequestURI())
            && request.getMethod().equalsIgnoreCase(RequestMethod.OPTIONS.name())){
            return;
        }
        if (currentId == null) {
        ...
        // 将sessionid设置请求头中，手机端用于保持会话一致
        response.setHeader(this.X_AUTH_TOKEN, idString);
        log.info("Set session ID cookie for session with id {}", idString);
    }
    ...
    public Serializable getSessionId(SessionKey key) {
        // 优先取请求头中的token，用于前后端分离-手机端
        if (WebUtils.isWeb(key)) {
            ServletRequest request = WebUtils.getRequest(key);
            HttpServletRequest httpServletRequest = (HttpServletRequest)request;
            String sid = httpServletRequest.getHeader(this.X_AUTH_TOKEN);
            if(StringUtils.isNotBlank(sid)
                && !"/captcha/captchaImage".equals(httpServletRequest.getRequestURI())) {
                return sid;
            }
        }
        ...
        if (id == null && WebUtils.isWeb(key)) {
            ...
            // HttpServletRequest httpServletRequest = (HttpServletRequest)request;
            // log.debug(httpServletRequest.getRequestURI() + "=====" +httpServletRequest.getMethod() + "=======" + id);
        }
        return id;
    }
