/** JavaBoot iframe v3.1.8 date:2020-05-04 License By http://www.javaboot.cn */
...
layui.define(['layer', 'table'], function (exports) {
    ...
    admin.events = {
        ...
        /* 打开修改密码弹窗 */
        changeRoleAuth: function () {
            var option = admin.util.deepClone($(this).data());
            admin.strToWin(option.window).layui.admin.open($.extend({
                id: 'layer-changerole', title: '切换角色',area:['500px', '300px'], shade: 0, url: option.url || 'page/tpl/tpl-changerole.html'
            }, admin.parseLayerOption(option)));
        },
        /* 退出登录 */
        logout: function () {
    ...
    /******************** begin: add by javaboot ***************/
    admin.info = function(msg){
        return layer.msg(msg);
    };
    admin.warning = function(msg){
        return layer.msg(msg, {icon:0});
    };
    admin.success = function(msg){
        return layer.msg(msg, {icon:1});
    };
    admin.error = function(msg){
        return layer.msg(msg, {icon:2});
    };
    admin.alert = function(msg){
        layer.alert(msg, {
    ...
    admin.confirm = function(msg, yes, no){//msg:询问信息， yes：点‘是’调用的函数， no:点‘否’调用的函数
        layer.confirm(msg, {
            icon: 3, title:'系统提示', skin:'layui-layer-admin', btn: ['确定','取消']
        }, function(index){
    ...
    admin.confirm1 = function(msg, yes){//msg:询问信息， yes：点‘是’调用的函数， no:点‘否’调用的函数
        layer.confirm(msg, {
            icon: 3, title:'系统提示', skin:'layui-layer-admin', btn: ['确定']
        }, function(index){
            if(typeof yes == 'function'){
                yes();
            }
    ...
    admin.prompt = function (title, href) {
    ...
    admin.dataURLtoBlob = function (dataurl) {
    ...
    admin.blobToDataURL = function (blob, callback) {
        var a = new FileReader();
        a.onload = function (e) { callback(e.target.result); }
        a.readAsDataURL(blob);
    };
    admin.upload = function(blob, success){
    ...
    admin.uploadFile = function(blob, success){
        var formData = new FormData();
        formData.append('cmd', 'upload');
        formData.append('target', 'A_');
        formData.append('upload[]',blob,new Date().getTime() + '.jpg');

        $.ajax({
            url: '/filemanager/connector', //用于文件上传的服务器端请求地址
            dataType:'json',
            method:"post",
            data: formData,
            processData: false,
            ...
            success:function(result){
                var url = "/filemanager/connector?cmd=file&target=" + result.added[0].hash;
                if(typeof success === 'function'){
                    success(url);
                }
            }
    ...
    admin.openDialog = function(title,url,width,height,tableName,submitBtn,success){
        ...
        layer.open({
            ...
            yes: function(index, layero){
                if(!tableName){//如果不传递table对象过来，按约定的默认id获取table对象
                    tableName = 'dataTable';
                }
                if(!submitBtn){
                    submitBtn = 'submitBtn';
                ...
                var iframeWindow = window['layui-layer-iframe' + index]
                    ,inputForm = layero.find('iframe').contents().find('#inputForm')
                ...
                iframeWindow.layui.form.on('submit(' + submitBtn + ')', function (data) {
                    ...
                    admin.req(action, field, function(data){
                        if (data.code == 0) {
                            if(tableName === 'dataTable'){
                                table.reload(tableName); //数据刷新
                            }else if(tableName === 'treeTable'){
    ...
    //打开对话框(查看)
    admin.openDialogView = function(title,url,width,height){
        var auto = true;//是否使用响应式，使用百分比时，应设置为false
        if(width.indexOf("%")>=0 || height.indexOf("%")>=0 ){
            auto =false;
        }
    ...
    //打开对话框(查看)
    admin.openDialogView2 = function(title,url,width,height,callback){
        var auto = true;//是否使用响应式，使用百分比时，应设置为false
        if(width.indexOf("%")>=0 || height.indexOf("%")>=0 ){
            auto =false;
        }
    ...
    //打开对话框(查看)
    admin.openDialogView3 = function(title,url,width,height){
        var auto = true;//是否使用响应式，使用百分比时，应设置为false
        if(width.indexOf("%")>=0 || height.indexOf("%")>=0 ){
            auto =false;
        }
        ...
        layer.open({
            ...
            success:function (layero, index) {
                var submitBtn = layero.find('iframe').contents().find('#submitBtn');
                $(submitBtn).hide();
            }
        });
    ...
    //打开对话框
    admin.openDialogSimple = function(title,url,width,height){
        var auto = true;//是否使用响应式，使用百分比时，应设置为false
        if(width.indexOf("%")>=0 || height.indexOf("%")>=0 ){
            auto =false;
        }
    ...
    admin.openUrl = function(title,url,width,height,callback){
        var auto = true;//是否使用响应式，使用百分比时，应设置为false
        if(width.indexOf("%")>=0 || height.indexOf("%")>=0 ){
            auto =false;
        }
    ...
    admin.openDialogSimpleView = function(title,url,width,height) {
        var auto = true;//是否使用响应式，使用百分比时，应设置为false
        if (width.indexOf("%") >= 0 || height.indexOf("%") >= 0) {
            auto = false;
        }
    ...
    admin.openDialogWithCustBtn = function(title,url,width,height,btnArr,tableName,submitBtn,success){
        var auto = true;//是否使用响应式，使用百分比时，应设置为false
        if(width.indexOf("%")>=0 || height.indexOf("%")>=0 ){
            auto =false;
        }
