...
public class SysDeptServiceImpl implements ISysDeptService {
     ...
     */
    @Override
    public String checkDeptIdUnique(SysDept dept) {
        String deptId = StringUtils.isNull(dept.getDeptId()) ? "-1" : dept.getDeptId();
        SysDept info = deptMapper.checkDeptIdUnique(deptId);
     ...
     */
    @Override
    public List<SysDeptSelectVO> selectDeptListById(String deptId) {
        List<SysDeptVO> sysDeptList =  deptMapper.selectDeptListById(deptId);
        List<SysDeptSelectVO> list = new ArrayList<>();
        if (CollectionUtils.isNotEmpty(sysDeptList)) {
            for (SysDeptVO ch : sysDeptList) {
                boolean hasChild = sysDeptList.stream().filter(e -> e.getAncestors().equals(ch.getAncestors() + "," + ch.getDeptId())).count() > 0;
                if (!hasChild) {
                    list.add(SysDeptSelectVO.builder().value(ch.getDeptId()).name(ch.getDeptName()).build());
                }
            }
        }
        return list;
