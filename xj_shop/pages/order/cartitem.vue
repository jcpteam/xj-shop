					...
					<view class="item-right">
						...
						<view class="item-last">
							<view class="left-price">
								...
								<view v-if="item.status != '1'" class="goods-invalid">
									商品已下架或删除，请删除该商品再下单
								</view>
							</view>
		...
		methods: {
				...
				uni.showModal({
					...
					success: function(res) {
						if (res.confirm) {
							...
							let url = '/shop/cartItem/delCartGoodsWithUserId?userId=' + self.userInfo.memberId;
							self.sendRequest({
								'url': url,
								'method': 'POST',
								'data': null
			...
			addGoodRemark(item) {
				...
				if (this.userCartMap && this.userCartMap[goodsId]) {
					...
					cartItemId = this.userCartMap[goodsId].itemId;
				}
				this.$jump('/pages/order/cartgoodremark?goodsId=' + item.goodsId + "&cartItemId=" + cartItemId + "&comment=" + comment);
			},
