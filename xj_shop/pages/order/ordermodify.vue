	...
	export default {
		...
		methods: {
			...
			getOrderDetail() {
				...
				let self = this;
				let url = '/shop/order/appList?pageNum=' + 1 + '&pageSize=' + 10 + '&orderId=' + this.orderId;
				this.sendRequest({
					'url': url,
					'data': null
				}, function(resp) {
			...
			submitModify() {
				...
				if (this.orderGoodList && this.orderGoodList.length > 0) {} else {
			...
			addNewGoods(){
				...
				uni.redirectTo({
					url:'/pages/order/ordermodifyadd'
				});
