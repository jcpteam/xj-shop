...
//			System.out.println("串口不存在!");
	...
	public void serialEvent(SerialPortEvent event) {
		try {
			...
			System.out.print("serialEvent[smtw] ");
			e.printStackTrace();
		}
	...
	public void changeMessage(byte[] message, int length) {
		...
		if(!"".equals(temp1)){
			if(!temp1.equals(new String(temp).trim())){
				...
				System.out.println("msg[" + numBytes + "]: [" + new String(temp).trim() +"]");
			}
		}else{
			temp1 = new String(temp).trim();
			notifyObservers(temp);
...
//					System.out.println("Failed to open port " + com.getName()+ e);
	...
	public static List<String> getAvailableSerialPortsStr(){
	 	List<String> ports = new ArrayList<>();
		HashSet<CommPortIdentifier> set = getAvailableSerialPorts();
		for(CommPortIdentifier commPortIdentifier:set){
			ports.add(commPortIdentifier.getName());
		}
		return ports;
	}

	public static  void start(String com, CopyOnWriteArraySet<Session> webSocketSet){
		HashMap<String, Comparable> params = new HashMap<String, Comparable>();
		params.put(SerialReader.PARAMS_PORT, com); // 端口名称
		...
		CommDataObserver bob = new CommDataObserver("xiangjia",webSocketSet);
//	  CommDataObserver joe = new CommDataObserver("素菜");
//	  sr.addObserver(joe);
		sr.addObserver(bob);
	}
}
