...
import com.javaboot.wechat.entity.Menu;
import com.javaboot.wechat.entity.MenuButton;
import com.javaboot.wechat.service.WxMenuService;
...
public class WxMenuController {
    ...
    private static final String prefix = "wechat/menu" ;

    @RequestMapping(value = "/data")
    @ResponseBody
    public AjaxResult data(String appid) {

        String wxMenuButton = wxMenuService.getWxMenuButton(appid);
        Menu menu = Menu.fromJson(wxMenuButton);
        System.out.println("data = " + menu);
        return AjaxResult.success(menu).put("success", true);
    }
    ...
    @GetMapping("/save")
    @ResponseBody
    public AjaxResult save(String appid, String menus) throws WxErrorException {
        JSONObject buttons = JSONUtil.createObj().putOpt("button", JSONUtil.parseArray(menus));
        System.out.println(buttons.toString());
        this.wxService.switchoverTo(appid);
        wxMenuService.saveAndRelease(appid, buttons.toString());

        return AjaxResult.success().put("success", true);
    }
...
//        menu.getButtons().add(button2);
     ...
     * @return 如果是个性化菜单，则返回menuid，否则返回null
     */
    @PostMapping("/createByJson/{appid}")
    @ResponseBody
     ...
     * </pre>
     */
    @GetMapping("/getSelfMenuInfo/{appid}")
    @ResponseBody
